...
var makeDispatcher = function (tag, isSynchronous) {
  ...
  var methods = dispatcherMethods.map(function (methodName) {
    ...
    return [methodName, function (...args) {
      ...
      var returnValue = NativeDatabaseBridge[name].apply(NativeDatabaseBridge, [tag].concat(_toConsumableArray(otherArgs)));
...
function () {
  ...
  _proto.unsafeSqlQuery = function unsafeSqlQuery(tableName, sql, callback) {
    ...
    this._dispatcher.query(tableName, sql, function (result) {
